...
/**
 * Register meta boxes for canonical URL and audio enclosure.
 */
...
/**
 * Render the audio enclosure meta box.
 *
 * @param WP_Post $post The current post object.
 */
...
function asdo_save_meta_boxes( $post_id ) {
	...
	if ( isset( $_POST['asdo_canonical_nonce_field'] ) &&
		wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['asdo_canonical_nonce_field'] ) ), 'asdo_canonical_nonce' ) ) {
		if ( isset( $_POST['canonical_href'] ) ) {
			update_post_meta( $post_id, 'canonical_href', esc_url_raw( wp_unslash( $_POST['canonical_href'] ) ) );
		}
		if ( isset( $_POST['canonical_name'] ) ) {
			update_post_meta( $post_id, 'canonical_name', sanitize_text_field( wp_unslash( $_POST['canonical_name'] ) ) );
		}
	}

	// Save enclosure fields.
	if ( isset( $_POST['asdo_enclosure_nonce_field'] ) &&
		wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['asdo_enclosure_nonce_field'] ) ), 'asdo_enclosure_nonce' ) ) {
		if ( isset( $_POST['enclosure_url'] ) ) {
			update_post_meta( $post_id, 'enclosure_url', esc_url_raw( wp_unslash( $_POST['enclosure_url'] ) ) );
		}
		if ( isset( $_POST['enclosure_size'] ) ) {
			update_post_meta( $post_id, 'enclosure_size', sanitize_text_field( wp_unslash( $_POST['enclosure_size'] ) ) );
		}
		if ( isset( $_POST['enclosure_type'] ) ) {
			update_post_meta( $post_id, 'enclosure_type', sanitize_text_field( wp_unslash( $_POST['enclosure_type'] ) ) );
		}
	}
}
